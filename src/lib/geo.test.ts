...
describe('normalizeGeography', () => {
  it('validates a known ZIP code and returns lat/lng', () => {
    const res = normalizeGeography({ locationType: 'zip', zipCode: '78701' });
  ...
  it('rejects invalid ZIP code', () => {
    const res = normalizeGeography({ locationType: 'zip', zipCode: '123' });
    expect(res.isValid).toBe(false);
    expect(res.errors).toContain('Invalid or missing 5-digit zipCode');
  });

  it('normalizes valid city and state casing', () => {
    const res = normalizeGeography({ locationType: 'city_state', city: ' austin ', state: ' tx ' });
    expect(res.isValid).toBe(true);
    expect(res.city).toBe('Austin');
    expect(res.state).toBe('TX');
  });

  it('rejects missing city/state', () => {
    const res = normalizeGeography({ locationType: 'city_state', city: 'Austin' });
    expect(res.isValid).toBe(false);
