  ...
  if (state?.success) {
    return (
      <div className="auth-success-box">
        ...
        <Link href="/login" className="auth-btn-secondary">Back to sign in</Link>
      </div>
    );
  }
...
// ─── Stage 2: Set new password using token ────────────────────────────────────
  ...
  return (
    <>
      ...
      <form action={action} className="auth-form">
        ...
        {state?.errors?.token && (
          <div className="auth-error-banner">{state.errors.token[0]}</div>
        )}
...
function ResetPasswordContent() {
  ...
  return (
    <div className="auth-shell">
      ...
      <style jsx>{`
        .auth-shell { min-height:100vh;display:flex;align-items:center;justify-content:center;background:#020617;padding:24px;position:relative;overflow:hidden; }
        ...
        .auth-card { position:relative;z-index:1;background:rgba(15,23,42,0.85);backdrop-filter:blur(24px);border:1px solid rgba(99,102,241,0.25);border-radius:20px;padding:40px;width:100%;max-width:420px;box-shadow:0 25px 50px rgba(0,0,0,0.5);animation:cardIn 0.4s cubic-bezier(0.16,1,0.3,1); }
        ...
        .auth-subtitle { font-size:14px;color:#64748b;margin:0 0 28px;line-height:1.5; }
        .auth-form { display:flex;flex-direction:column;gap:18px; }
        .auth-error-banner { background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:8px;padding:10px 14px;font-size:13px;color:#f87171; }
        ...
        .auth-btn-primary { width:100%;background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;border:none;border-radius:10px;padding:13px;font-size:15px;font-weight:600;cursor:pointer;transition:opacity 0.2s; }
        .auth-btn-primary:hover:not(:disabled) { opacity:0.9; }
        .auth-btn-primary:disabled { opacity:0.6;cursor:not-allowed; }
        .auth-btn-secondary { display:inline-block;margin-top:16px;background:rgba(99,102,241,0.15);color:#818cf8;border:1px solid rgba(99,102,241,0.3);border-radius:10px;padding:11px 24px;font-size:14px;font-weight:600;cursor:pointer;text-decoration:none;transition:background 0.2s; }
        .auth-btn-secondary:hover { background:rgba(99,102,241,0.25); }
        ...
        .auth-success-box { text-align:center;padding:16px 0; }
        .auth-success-icon { font-size:48px;margin-bottom:16px; }
        .auth-success-title { font-size:22px;font-weight:700;color:#f1f5f9;margin:0 0 12px; }
        .auth-success-msg { font-size:14px;color:#94a3b8;line-height:1.6;margin:0 0 24px; }
      `}</style>
    </div>
  );
}
